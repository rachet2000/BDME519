function f = ranpdf (type, x, p1, p2,p3)
% generates proability densities for various distirbutions
%
% usage:
%          f = ranpdf (type, x, p1, p2, p3 )
%
% where
...
%
% 
% 20 Oct 94 Corrected error in Student t
% 21 Oct 93 Use gammaln rather than gamma to avoid overflow errors
% {{{ Normal
...
if (strcmp(type,'normal')),
  ...
  %
  if (nargin < 4),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''normal'',x,mean,variance');
    f=NaN;
    return
  end
  mean =p1;
...
elseif(strcmp(type,'chi-square')),
  ...
  if (nargin < 3),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''chi-square'',x,degrees_of_freedom');
    f=NaN
    return
  end
  if (p1 <0),
...
elseif(strcmp(type,'student-t')),
  ...
  if (nargin < 3),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''student-t'',x,degrees_of_freedom');
    f=NaN;
    return
  end
  dof =p1;
...
elseif(strcmp(type,'F-distribution')),
  ...
  if (nargin < 4),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''F-distirbution'',x,n1,n2');
    f=NaN;
    return
  end
  if (p1<=0) | (p2<=0),
  ...
  if any(x<=0),
    disp('ERROR: ranpdf: Bad value. x must be > 0')
    f = NaN;
    return
  end 
...
elseif (strcmp(type,'log-normal')),
  ...
  if (nargin < 5),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''log-normal'',x,mean,var,offset');
    f=NaN;
    return
  end
  
  if any(x <= offset),
    disp('ERROR: ranpdf: Bad value. x must be > offset')
    f = NaN;
    return
  end
  x1=x-offset;
...
elseif (strcmp(type,'rectangular')),
  ...
  if (nargin < 4),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''rect'',x,xmin,xmax');
    f=NaN;
    return
  end
  xmin=p1;
...
elseif(strcmp(type,'exponential')),
  ...
  if (nargin < 3),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''exponential'',x,mean');
    f=NaN;
    return
  end

    if (any(x<0)),
    disp('ERROR: x >0 for exponential')
    f=NaN;
    return
  end
 %
...
elseif(strcmp(type,'binomial')),
  ...
  if (nargin <4),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''binomial'',x,number-of-event,probability');
    f=NaN;
    return
  end
...
elseif(strcmp(type,'Poisson')),
  ...
  if (nargin < 3),
    disp('ERROR: ranpdf  not enough parameters specified');
    disp('Usage is: f = ranpdf(''Poisson'',x,mean');
    f=NaN;
    return
  end
